 ...
 *     and set to LUNDECL and a new label number.
 *
 * expr = lookup(name)
 ...
 * is generated with the newlabel function; it starts out as LUND.
 * A label can be placed at the current location with putlabel.  This
 * updates the label value to class LLABEL.  Attempts to place a label
 * again are rejected and yield an error message.  setlabel behaves
 * similar but sets the label's value to be whatever is emitted next.
 */
extern void newlabel(struct expr *);
extern void putlabel(struct expr *);
extern void setlabel(struct expr *);
